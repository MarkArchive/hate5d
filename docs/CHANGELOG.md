...
- `char.nim` with a lot of keycodes (`SDLSK_*`, * can be a char, number, symbol and special keys (e.g.: F1, return))
    -  DISCLAIMER: `char.nim` is not complete, in v0.1.2, it will be complete (e.g. (characters in v0.1.2): § + = ¬ ¢ pagedown)
- `logic.nim`: mouse state queries (`mousePos`, `mouseLeft`, `mouseRight`, `mouseMiddle`)
- `ecs.nim`: initial ECS module (`Thing`, `Entity`, `Concept`, `Tag`, `Group`, registries, `update`/`react` loops)
- `Object` changed from `object` to `ref object of RootObj`; `Thing` inherits from `Object` (removed duplicate fields)
- `drawObject` and `deleteObject` no longer need `var` param (ref semantics)
- `hatelib.nim` now exports `ecs` module
- ECS module added to `hatelib.nim` import/export chain
