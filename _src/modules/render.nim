...
proc initFrame*(win: WindowInfoMake): HateResult[bool] =
    ...
    # cria um render, limpa o render e retorna normalmente
...
proc drawSprite*(win: WindowInfoMake, path: string, x, y, w, h: int32): HateResult[bool] =
    ...
    if not textureCache.hasKey(path):
        ...
        # coloca a texture no textureCache para não ficar renderizando toda hora direto do ssd
        textureCache[path] = tex

    var dst = rectOf(x, y, w, h)
    discard sdlRenderCopy(win.sdlRen, textureCache[path], nil, addr dst)
    return HateResult[bool](ok: true, val: true)